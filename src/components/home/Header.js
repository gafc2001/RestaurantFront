...
import {   shoppingReducer,
  shoppingInitialState, } from "../../reducers/shoppingReducer";

export const Header =({filtCategory})=>{
    ...
    return(
          ...
          <div className="header-info">
            <div className="page-info">
              <h2 className="page-name">{localStorage.getItem("username")}</h2>
          ...
          {category && category.map((item, index) => (
            <FilterCategory key={index} data={item} filtCategory={filtCategory} />
         ))}
         </ul>
         </div>
        </header>
